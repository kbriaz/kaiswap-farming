...
const FarmCards: React.FC = () => {
  ...
  const kaiIndex = farms.findIndex(
    ({ tokenSymbol }) => tokenSymbol === 'KAI',
  )

  const kaiPrice =
    kaiIndex >= 0 && stakedValue[kaiIndex]
      ? stakedValue[kaiIndex].tokenPriceInWeth
      : new BigNumber(0)

  const BLOCKS_PER_YEAR = new BigNumber(2336000)
  const KAI_PER_BLOCK = new BigNumber(100)
...
const FarmCard: React.FC<FarmCardProps> = ({ farm }) => {
  ...
  }, [kai, lpTokenAddress, account, setHarvestable])

  const poolActive = true // startTime * 1000 - Date.now() <= 0

  return (
    <StyledCardWrapper>
      {farm.tokenSymbol === 'KAI' && <StyledCardAccent />}
      <Card>
        <CardContent>
          <StyledContent>
